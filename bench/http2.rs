...
fn bench_server_single_thread(c: &mut Criterion) {
    bench::bench_server_single_thread(c, HTTP_MODE, Tls::Disabled, ADDR, NUM_REQUESTS_TO_SEND)
        .expect("Failed to run single-threaded HTTP/2 benchmark server")
}

#[inline]
fn bench_server_multi_thread(c: &mut Criterion) {
    bench::bench_server_multi_thread(c, HTTP_MODE, Tls::Disabled, ADDR, NUM_REQUESTS_TO_SEND)
